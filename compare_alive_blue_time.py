...
def main():
    ...
    for f, c, l in zip(filelist, colorlist, legend_list):
        ...
        for idx in range(len(alive_blue_ratio) - window + 1):
            ...
            averaged_alive_blue_ratio.append(
                np.mean(alive_blue_ratio[idx:idx + window])
            )

        averaged_correct_time = np.array(averaged_correct_time)
        averaged_alive_blue_ratio = np.array(averaged_alive_blue_ratio)

        plt.xlabel('learning time [hours]')
        plt.ylabel('mean alive blue ratio')

        plt.plot(averaged_correct_time / 3600, averaged_alive_blue_ratio, linestyle='solid',
    ...
    # plt.yscale('log')
    plt.title(f'Moving average of alive blue ratio vs Learning time, window={window}')
    plt.grid(which="both")
    plt.minorticks_on()
    plt.legend()
