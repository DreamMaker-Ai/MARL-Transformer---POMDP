...
def main():
    ...
    for f, c, l in zip(filelist, colorlist, legend_list):
        ...
        averaged_wall_time = []
        averaged_step = []
        averaged_blues_force = []

        for idx in range(len(blues_force) - window + 1):
            ...
            averaged_blues_force.append(
                np.mean(blues_force[idx:idx + window])
            )

        averaged_step = np.array(averaged_step)
        averaged_blues_force = np.array(averaged_blues_force)

        plt.xlabel('learning steps [k]')
        plt.ylabel('mean remaining effective force of blues')

        plt.plot(averaged_step / 1000, averaged_blues_force, linestyle='solid', color=c,
    ...
    # plt.yscale('log')
    plt.title(f'Moving average of remaining force of blues vs Learning steps, winow={window}')
    plt.grid(which="both")
    plt.minorticks_on()
    plt.legend()
