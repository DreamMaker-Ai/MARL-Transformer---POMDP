...
def main():
    ...
    for f, c, l in zip(filelist, colorlist, legend_list):
        ...
        plt.plot(averaged_step / 1000, averaged_red_ratio, linestyle='solid', color=c, alpha=0.7,
                 linewidth=1, label=l)
    ...
    # plt.yscale('log')
    plt.title(f'Moving average of alive red ratio vs Learning steps, window={window}')
    plt.grid(which="both")
    plt.minorticks_on()
    plt.legend()
